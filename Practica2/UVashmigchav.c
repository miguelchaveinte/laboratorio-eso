...
void ejecutaRedireccion(char * Comando){
	...
	if(strstr(Comando,Redireccion)!=NULL) tieneRedireccion=true;	// Comprobar que tieneel caracter > de redireccion

	cadenaIndependiente=strtok(Comando,Redireccion);
	i=0;
...
void ejecutaComando(char * Comando,char * fich, bool hayRedireccion){
	...
	if(Argumentos[0]!=NULL){ 
		...
		//TODO:SI FALLAN LAS CONDICIONES MENSAJE ERROR
		...
		//no deberia entrar luego en lo otro de ejecutar el comando
